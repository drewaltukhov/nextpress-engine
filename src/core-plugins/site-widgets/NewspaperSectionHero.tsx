...
export interface NewspaperSectionHeroProps {
  type: "pillar" | "topic";
  pillarIds: number[];
  topicSlugs: string[];
  showAllTab: boolean;
  ...
  // See NewspaperHero / NewspaperSection — same Overlays vs Cards
  // dichotomy; applied to the featured slot here too (no
  // "first 50%" exception — that exception only exists for the
  // standalone NewspaperHero widget).
...
function buildTabs(
  props: NewspaperSectionHeroProps,
  md: PuckMetadataShape,
): NewspaperTab[] {
  const tabs: NewspaperTab[] = [];
  if (props.type === "pillar") {
    // Empty array is the PillarMultiPicker "all checked" sentinel —
    // surface it as a single synthetic "All" tab so the widget stops
    // rendering an empty section on its default state. The scope's
    // empty `scopes` array tells the SSR collector to fetch all spikes
    // (kind: "all", allType: "pillar", keys: []).
    if (props.pillarIds.length === 0) {
      tabs.push({
        key: "section-hero:all-spikes",
        label: "All",
    ...
    if (props.topicSlugs.length === 0) {
      tabs.push({
        key: "section-hero:all-topic-posts",
        label: "All",
...
export const NewspaperSectionHero: ComponentConfig<NewspaperSectionHeroProps> = {
  label: "Hero + Side List",
  fields: {
    type: {
      type: "radio",
      label: "Scope by",
    ...
    limit: { type: "number", label: "Posts per tab", min: 4, max: 8 },
    showDate: {
      type: "radio",
      label: "Show date",
      options: [
  ...
  resolveFields: (data, { fields }) => {
    ...
    const scopes =
      t === "pillar"
        ...
        : Array.isArray(data.props?.topicSlugs)
          ...
          : 0;
    const hide = new Set<keyof NewspaperSectionHeroProps>();
    ...
    return Object.fromEntries(
      Object.entries(fields).filter(
        ([key]) => !hide.has(key as keyof NewspaperSectionHeroProps),
      ),
    ) as typeof fields;
  },
  render: (props) => {
    ...
    if (puck?.isEditing && md.themeBuilder) {
      return <BuilderCard name="NewspaperSectionHero" title="Hero + Side List" description={"One big featured card on the left, 4 thumbnail rows on the right. Section heading + tab strip."} />;
    }
    const tabs = buildTabs(props, md);
    if (tabs.length === 0) return <section data-np-toc-skip="" />;

    const safeLimit = Math.max(4, Math.min(8, Math.floor(limit ?? 5)));
    ...
    const config: NewspaperWidgetConfig = {
      widgetId: `newspaper-section-hero-${props.type}-${tabs[0]!.key}`,
    ...
    return (
      <section
        className="np-newspaper-widget np-newspaper-section-hero not-prose mb-8"
        data-np-toc-skip=""
      ...
      >
        ...
        {/* Prev/next arrows — mounter wires by data attribute regardless
            of layout. Disabled at SSR so an unhydrated page can't
            advertise pagination it can't perform. */}
        <div
...
export const NewspaperSectionHeroBlock: Omit<RegisteredBlock, "source"> = {
  name: "NewspaperSectionHero",
  config: NewspaperSectionHero,
  surfaces: [
    "sidebar",
    "post-content",
    "page-content",
