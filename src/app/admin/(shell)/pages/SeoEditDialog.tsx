...
import type { PageDetail, PageRobots } from "@core-plugins/pages";
import {
  getPageDetail,
  getPageSchemaPreview,
  updatePageSeoAction,
  type PageSchemaPreview,
} from "./actions";

interface Props {
  /** When set, the dialog opens loading the given page's SEO state. */
  pageId: number | null;
  onClose: () => void;
}
...
export function SeoEditDialog({ pageId, onClose }: Props) {
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Quick edit for search and social-share metadata. The full editor lives on the page edit screen.
          </DialogDescription>
        </DialogHeader>
        {open && pageId !== null ? (
          <SeoEditForm key={pageId} pageId={pageId} onClose={onClose} />
        ) : null}
      </DialogContent>
    </Dialog>
  );
...
function SeoEditForm({ pageId, onClose }: { pageId: number; onClose: () => void }) {
  ...
  const [seoRobots, setSeoRobots] = useState<PageRobots>("index,follow");
  const [seoOgImage, setSeoOgImage] = useState("");
  const [seoExcludeFromSitemap, setSeoExcludeFromSitemap] = useState(false);

  // Schema preview is fetched on demand — only when the user first opens
  // the Schema Checkup tab. Saves a Puck-data parse + a few setting reads
  // for the common case where the admin only edits title/description.
  const [activeTab, setActiveTab] = useState("search");
  const [schemaPreview, setSchemaPreview] = useState<PageSchemaPreview | null>(null);
  const [schemaLoading, setSchemaLoading] = useState(false);

  // Load fresh — we don't trust the row's seo_title/description from the
  // list (the list query elides og_image/canonical/robots). Lint rule
  // wants the useState declarations above this effect so the closure
  // captures setters declared higher in the function body.
  useEffect(() => {
    let cancelled = false;
    (async () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const preview = await getPageSchemaPreview(pageId);
      if (cancelled) return;
  ...
  }, [activeTab, pageId, schemaPreview]);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await updatePageSeoAction(page.id, {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500">
        Editing SEO for <span className="font-medium text-slate-700">{page.title}</span>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col">
        ...
        {/* Same tab-row treatment as AdminSection — scroll horizontally on
            overflow rather than wrapping. The dialog is sm:max-w-2xl, so
            this only kicks in on very narrow phone widths, but it keeps
            the visual consistent with the admin pages. */}
        ...
        <TabsContent value="search" className="space-y-4">
          <div>
            ...
            <input
              ...
              type="text"
              maxLength={200}
              placeholder={`Falls back to the page title (${page.title})`}
              value={seoTitle}
          ...
          <div>
            ...
            <input
              ...
              type="url"
              maxLength={500}
              placeholder={`https://yoursite.example/${page.slug}`}
              value={seoCanonical}
          ...
          <div>
            ...
            <Select value={seoRobots} onValueChange={(v) => setSeoRobots(v as PageRobots)}>
          ...
          <div className="flex items-start justify-between gap-4 pt-2 border-t border-slate-100">
            <div className="min-w-0">
              ...
              <div className="text-xs text-slate-500">
                Per-page override. Drops this page from <code className="font-mono">/sitemap.xml</code>{" "}
                even when the site-wide Pages toggle is on.
              </div>
            </div>
            <Switch
        ...
        <TabsContent value="social" className="space-y-4">
          <div>
            ...
            />
            <p className="mt-1 text-xs text-slate-500">
              Shown when this page is shared on Facebook, LinkedIn, Slack, etc. Recommended: 1200×630.
            </p>
          </div>
        </TabsContent>
...
// ─── Schema Checkup ─────────────────────────────────────────────────────
//
// Renders the JSON-LD nodes the public route would emit for this page,
// shaped as collapsible groups (one per @type). Source of truth is
// `getPageSchemaPreview` which calls the same `buildPageJsonLdNodes`
...
function SchemaCheckup({
  ...
  loading: boolean;
  preview: PageSchemaPreview | null;
}) {
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12 text-sm text-slate-500">
    ...
    <div className="space-y-3">
      ...
      {suppressionReason === "noindex" && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-slate-700">
          ...
          This page is set to <code className="font-mono">noindex</code>, so
      ...
      {nodes.length === 0 && suppressionReason === null && (
        <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-6 text-center">
          ...
          <p className="mt-1 text-xs text-slate-500">
            This page has no FAQ blocks, no Article schema selected, and no
            site-wide breadcrumb context. Pick a schema in the page&apos;s
...
function SchemaNode({ node, index }: { node: object; index: number }) {
  ...
  // doesn't explode vertically on a page with five schema types.
  return (
    <details
      open={index === 0}
