...
import { restorePageRevisionAction } from "../../../revisions/actions";
...
export async function generateMetadata({
  ...
  const page = await getPage(db(), Number(id));
  return { title: page ? `History — ${page.title}` : "History" };
}
...
export default async function PageHistoryPage({
  ...
  const curBlocks = parseBlocks(page.contentJson ?? "{}");
  const curRoot = parseRoot(page.contentJson ?? "{}");
  const diff = snap ? computeBlockDiff(revBlocks, curBlocks) : [];

  const titleChanged = snap && snap.title !== page.title;
    ...
    <div>
      <div className="mb-6">
        ...
        <h1 className="text-xl font-semibold text-brand-navy">
          History — <span className="font-normal text-slate-600">{page.title}</span>
        </h1>
      </div>
                ...
                return (
                  <li key={rev.id} className={`rounded-xl border bg-white shadow-sm transition-shadow hover:shadow-md ${isSelected ? "border-brand-green ring-1 ring-brand-green/30" : "border-slate-200"}`}>
                    ...
                    <div className="px-4 pb-3">
                      <form action={restorePageRevisionAction.bind(null, pageId, rev.id)}>
              ...
              {titleChanged && (
                <div className="px-5 py-3 border-b border-slate-200 space-y-1">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <span className="flex-1 text-sm text-green-900 font-medium bg-green-50 px-2 py-1 rounded">{page.title}</span>
                  </div>
                </div>
              )}
                  ...
                  {diff.map((row, i) => {
                    ...
                    if (row.kind === "del") return (
                      <div key={i} className="bg-red-50">
                        <RestoreBar kind="page" contentId={pageId} newContentJson={computeRestoredContent(diff, i, curRoot)} label="Block removed" accent="red" />
                    ...
                    if (row.del.type === "RichText" && row.ins.type === "RichText") {
                      ...
                      return (
                        <div key={i}>
                          <RestoreBar kind="page" contentId={pageId} newContentJson={computeRevertedContent(diff, i, curRoot)} label="Block changed" accent="slate" />
                                  ...
                                  return (
                                    <div key={si} className="relative group/para">
                                      ...
                                      <form action={restoreBlockAction} className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover/para:opacity-100 transition-opacity">
                                        ...
                                        <input type="hidden" name="contentId" value={String(pageId)} />
